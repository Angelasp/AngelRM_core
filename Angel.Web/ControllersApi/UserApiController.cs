...
* 描述说明 ：用户管理API控制器
* 创建信息 : create by QQ：815657032、709047174 Email:Angel_asp@126.com on 2023-10-09
...
**************************************************************************/
    ...
    public class UserApiController : BaseApiController
    {
        ...
        public UserApiController(IDataService _queryService, IHttpContextAccessor accessor, ICacheService cache) : base(_queryService, accessor)
        {
            this._cache = cache;
        }
        // GET api/userapi/get
        [HttpGet]
        public async Task<MessageModel<List<UserInfo>>> Get()
        {
            try
            {
                ...
                string data = QueryService.GetData(list, "1_2");
                List<UserInfo> model = JsonConvert.DeserializeObject<List<UserInfo>>(data);
                return Success<UserInfo>(model);  
            }
            catch (Exception er)
            {
                FileLog.WriteLog("Error：调用Angel.ControllersApi/ControllerApi/UserApiController/Get()方法," + er.ToString());
                return Failed<List<UserInfo>>();
            }
        }
        ...
        public async Task<MessageModel<string>> Post([FromBody] string value)
        {
            ...
            try
            {
                ...
                {
                    return Success<string>(QueryService.GetData(list, serverName));
                }
                else
                {
                    string result = QueryService.InsertBatchCheck(jArray, serverName);
                    JObject  resultObject = JObject.FromObject(JsonConvert.DeserializeObject(result));
                    int codeId =  Convert.ToInt32(resultObject["code"]["id"]);
                    if (codeId == 1)
                    {
                        return Success<string>(resultObject["code"]["msg"].ToString());
                    }
                    else {

                        resultData.code = codeId;
                        resultData.msg = resultObject["code"]["msg"].ToString();
        ...
        /// <returns></returns>
        public async Task<MessageModel<List<Tree_t>>> GetTBTreeView(int userid)
        {
            try
            {
                ...
                return Success<Tree_t>(list, "查询成功");
        ...
        /// <returns></returns>
        public async Task<MessageModel<string>> GetUserIndicatorView(int userid)
        {
            ...
            try
            {
                ...
                if (indicatorlist.Trim().Length > 2)
                {
                    ...
                    return Success<string>(QueryService.GetData(Indicatorids, "1_15"));
                }
                else
                {
                    return Failed<string>();
        ...
        public async Task<MessageModel<string>> PostUpPwd([FromBody] string value)
        {
            ...
            try
            {
                ...
                return Success<string>(QueryService.UpdatePassWord(jArray, serverName));
            }
            catch (Exception er)
            {
