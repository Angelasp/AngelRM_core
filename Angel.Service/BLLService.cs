...
namespace Angel.Service
{
    /*************************************************************************
       ...
       * 描述说明 ：执行语句操作
       * 
       * 创建信息 : create by QQ：815657032、709047174  E-mail:Angel_asp@126.com on 2023-09-10
    ...
    public class BLLService : IDataService
    {
        ...
        private readonly IApplicationSettingService _applicationSettings;
        /// <summary>
        /// xml 缓存
        /// </summary>
        Dictionary<string, Dictionary<Dictionary<string, string>, Dictionary<string, string>>> collection = null;

        public BLLService(IDatabase _database, IApplicationSettingService applicationSettings)
        {
            Database = _database;
            ...
            if (collection == null)
            {
                collection = _applicationSettings.GetSettingValue("key") as Dictionary<string, Dictionary<Dictionary<string, string>, Dictionary<string, string>>>;
            }
        }
        ...
        //直接返回DataTable
        public DataTable GetDataTable(Newtonsoft.Json.Linq.JObject param, string serverName)
        {
                    ...
                    if (val.Count > 0)
                    {
                        ...
                        foreach (var item in val)
                        {
                            ...
                            if (repSql != null && repSql.Count > 0)
                            {
                                foreach (var sql in repSql)
                                {
                                    result = Database.SelectDataTable(sql.Value);
                                }
                            }
